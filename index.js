...
function refreshItemsList() {
    ...
    container.querySelectorAll("[data-item]").forEach(function(el) {
        ...
        // 拖拽支持
        el.draggable = true;
        el.ondragstart = function(e) { e.dataTransfer.setData("text/plain", el.dataset.item); el.style.opacity = "0.4"; };
        el.ondragend = function() { el.style.opacity = "1"; };
    });
    // 分区标题作为 drop target
    container.querySelectorAll(".tlg-section-title").forEach(function(title) {
        title.ondragover = function(e) { e.preventDefault(); title.style.background = "rgba(255,255,255,0.1)"; };
        title.ondragleave = function() { title.style.background = ""; };
        title.ondrop = function(e) {
            e.preventDefault(); title.style.background = "";
            var itemName = e.dataTransfer.getData("text/plain");
            if (!itemName) return;
            var holderName = title.textContent.replace(/（.*?）/, "").trim();
            if (!holderName || holderName === "未分类") return;
            var w = currentWorldId && worlds[currentWorldId] ? worlds[currentWorldId] : null;
            if (!w) return;
    ...
    function refreshWorlds() {
        ...
        container.innerHTML = ids.map(function (wid) {
            ...
            return '<div style="background:#050508;border:1px solid ' + (isCurrent ? "#ffffff" : "#2a2a3a") + ';border-radius:4px;padding:12px;margin-bottom:10px;">' +
                ...
                '<button type="button" class="tlg-btn tlg-btn-danger tlg-worlds-clear-summaries" data-wid="' + wid + '" style="writing-mode:horizontal-tb;white-space:nowrap;width:auto;height:auto;">清空档案</button>' +
                '</div></div>';
        }).join("");
