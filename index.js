...
(function () {
    ...
    function jumpToNode(nodeId) {
        ...
        var st = getST();
        // 跳转前总结：只要有可见消息+API配置+开关开着就触发，不依赖 turnsSinceAnchor
    ...
    function runCatchupSummary() {
        ...
        var batches = [];
        for (var k = 0; k < uncovered.length; k += batchSize) { batches.push(uncovered.slice(k, k + batchSize)); }
    ...
    function ensurePanelBuilt() {
        ...
        panel.innerHTML =
            ...
            '<button type="button" class="tlg-btn tlg-btn-primary" id="tlg-worlds-import" style="writing-mode:horizontal-tb;white-space:nowrap;width:auto;height:auto;">凝望异世界</button></div>' +
            ...
            '<label class="tlg-label">或强制指定</label><input class="tlg-input" id="tlg-model-manual" value="' + escHtml(s.model || "") + '" /></div>' +
            ...
            '<label class="tlg-label">联想提示词</label><textarea class="tlg-textarea" id="tlg-vec-prompt">' + escHtml(s.vectorPrompt || "") + '</textarea></div>' +
        ...
        try {
            ...
            if (ctx1 && ctx1.eventSource && ctx1.eventTypes) {
                ctx1.eventSource.on(ctx1.eventTypes.MESSAGE_RECEIVED, function () {
                    if (!isEnabled()) return;
                    // 每次收到消息先刷新设置，防止切换API等导致globalApi过期
                    loadGlobalApi();
                    ...
                    if (globalApi.autoMode && state.turnsSinceAnchor >= (globalApi.autoInterval || 10)) {
                        ...
                        saveTurnsCounter();
                        toast("⚙ 自律模式触发（" + (globalApi.autoInterval || 10) + " 步）");
                        runSummary(true);
                    }
